...
export default function ClientAccount() {
    ...
    const fetchUserData = async () => {
      try {
        ...
        setUsernameData({
          ...
          lastChanged: user?.usernameLastChanged ? new Date(user.usernameLastChanged) : null,
          canChange: !user?.usernameLastChanged || 
            (new Date().getTime() - new Date(user.usernameLastChanged).getTime() > 365 * 24 * 60 * 60 * 1000)
        });
      } catch (error) {
  ...
  // Update username data when user changes
  useEffect(() => {
    if (user) {
      setUsernameData({
        username: user.username || '',
        lastChanged: user.usernameLastChanged ? new Date(user.usernameLastChanged as any) : null,
        canChange: !user.usernameLastChanged || 
          (new Date().getTime() - new Date(user.usernameLastChanged as any).getTime() > 365 * 24 * 60 * 60 * 1000)
      });
    }
  }, [user]);
